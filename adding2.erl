...
%% The definition of an 'adding2' state as a binary string of 22 bits
...
trans(?state(C, _, X2, ?Q, P2), 0) when C < 200 -> ?state(C, C, X2, ?R, P2);
trans(?state(C, X1, _, P1, ?Q), 1) when C < 200 -> ?state(C, X1, C, P1, ?R);
