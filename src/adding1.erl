-module(adding1).
-export([initial/0, next/1]).

%% The definition of an 'adding1' state as a binary string of 22 bits
-define(state(C, X1, X2, P1, P2), <<C:6, X1:6, X2:6, P1:2, P2:2>>).
...
trans(?state(C, _, X2, ?Q, P2), 0) when C < 20 -> ?state(C, C, X2, ?R, P2);
trans(?state(C, X1, _, P1, ?Q), 1) when C < 20 -> ?state(C, X1, C, P1, ?R);
...
%% Given a state, this function returns a list containing the accessible states from the given state
next(S) -> [X || X <- [trans(S, 0), trans(S, 1)], X /= none].

%% Returns the initial state of the 'adding1' model
